...
extension View {
    ...
    ///
    /// - Parameters:
    ///   - name: The name of the font. If `nil`, the system font will be used.
    ///   - size: The size of the font.
    ///
    /// - Returns: A view with the custom font applied.
    nonisolated public func font(_ name: FontName? = nil, size: CGFloat) -> some View {
        if let name = name {
            return self.font(.custom(name.rawValue, size: size))
        } else {
